...
export function DestinationCard({ card, index }: DestinationCardProps) {
  ...
  const getStarRating = (score: number) => {
    const stars = Math.round((score / 20) * 2) / 2; // Round to nearest 0.5
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <h3 className="text-lg font-semibold leading-tight text-stone-900 font-serif">{card.destinationName}</h3>
        <span className={`
          text-[10px] uppercase tracking-wide px-3 py-1 rounded-full whitespace-nowrap font-medium
          ${isWinner ? 'bg-amber-100/80 text-amber-700 border border-amber-200' : 'bg-stone-100/80 text-stone-600 border border-stone-200'}
        `}>
          {matchLabel}
        </span>
      </div>
      ...
      <div className="text-center p-4 bg-stone-100/90 border border-stone-200 rounded-lg">
        <div className="text-[10px] uppercase tracking-wide text-stone-400 font-medium mb-2">Match Score</div>
      ...
      <div className="space-y-3">
        {categories.map((category, catIndex) => (
          <motion.div
            ...
            transition={{ delay: (index * 0.1) + (catIndex * 0.05), duration: 0.6, ease: "easeOut" }}
            className="flex items-center gap-3"
          >
            <span className="text-sm w-5 flex-shrink-0">{category.icon}</span>
            <span className="text-xs text-stone-600 w-20 flex-shrink-0 tracking-wide">
              {category.label}
            </span>
            <div className="flex-1 h-1.5 bg-stone-200 rounded-full overflow-hidden">
              <div 
                className={`h-full rounded-full transition-all duration-600 ${colors.bar}`}
